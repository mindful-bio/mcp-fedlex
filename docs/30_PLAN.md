...
- [x] Service-to-Service-mTLS + Default-Deny-NetworkPolicy (ADR-005). Reader ↔ Quota-Redis (einzige interne Cluster-Kante im Direct-Fetch-Stand) gegenseitig authentifiziert: anwendungsseitiges mTLS (`fedlex-store::RedisTlsConfig`, `RedisQuotaBackend::connect_with_tls`, `mcp-reader::main::build_quota_backend`, redis_tls-Unittests grün), Redis nur auf TLS-Port mit Client-Zwang + `requirepass`, Material als SealedSecret via `gen-redis-mtls.sh`, Default-Deny-NetworkPolicy (alles in `k3-infra`). Cluster ohne Mesh/cert-manager → anwendungsseitig statt Mesh (ADR-005 erlaubt).
...
- [x] Oxigraph-Backup/Restore (Backlog B-1). `OxigraphCorpus::dump_to_string`/`restore_from_str` (in-process), deterministischer, escapeter Schnappschuss; append-only-treuer Roundtrip (bi-temporale Aufloesung, Gueltigkeits-/Transaktionszeit, Mehrfach-Historie pro ELI erhalten, byte-gleicher Re-Dump). cargo-test-beweisbar: 8 Unit-Tests in `oxigraph_corpus.rs`.
...
| M10 Härtung | weitgehend | Health-Endpunkte (5 Tests), Server-Komposition mit echtem Socket (3 Tests) und Cache-Warmup gegen Stampede (5 Tests) erledigt. K8s-Manifeste in `k3-infra` (Reader-Deployment mit drei Proben, Redis, Default-Deny-NetworkPolicy, Argo-Application), client-validiert. mTLS Reader↔Redis umgesetzt (anwendungsseitig, ADR-005; `redis_tls`-Unittests grün, Manifeste + `gen-redis-mtls.sh` in `k3-infra`). Backup/Restore + Schema-Versionierung jetzt cargo-test-beweisbar (`dump_to_string`/`restore_from_str` + `SCHEMA_VERSION`-Guard, 8 Tests in `oxigraph_corpus.rs`). Offen bleibt nur noch der In-Cluster-Negativtest (nicht-authentifizierter Zugriff), der reine Infra ist |
